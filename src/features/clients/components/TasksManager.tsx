...
export function TasksManager({ clientId, initialTasks = [] }: TasksManagerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const taskData = {
      ...form,
      dueDate: form.dueDate ? toLocalISOString(parseDateInput(form.dueDate)) : null,
    }
      ...
      <div className="relative min-h-screen bg-linear-to-br from-slate-50 via-blue-50/30 to-slate-100 dark:from-slate-950 dark:via-blue-950/20 dark:to-slate-900">
        ...
        <div className="relative flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-400">Carregando tarefas...</p>
          </div>
        </div>
      </div>
    )
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative group">
              ...
              <div className="relative bg-amber-50/80 backdrop-blur-sm dark:bg-amber-950/30 rounded-2xl p-5 border border-amber-200/50 dark:border-amber-800/50 hover:-translate-y-1 transition-all duration-200">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-amber-700 dark:text-amber-400">A Fazer</p>
                  <p className="text-2xl font-bold text-amber-900 dark:text-amber-300">{stats.todo}</p>
                </div>
              </div>
            </div>
            ...
            <div className="relative group">
              ...
              <div className="relative bg-blue-50/80 backdrop-blur-sm dark:bg-blue-950/30 rounded-2xl p-5 border border-blue-200/50 dark:border-blue-800/50 hover:-translate-y-1 transition-all duration-200">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-blue-700 dark:text-blue-400">Em Progresso</p>
                  <p className="text-2xl font-bold text-blue-900 dark:text-blue-300">{stats.doing}</p>
                </div>
              </div>
            </div>
            ...
            <div className="relative group">
              ...
              <div className="relative bg-green-50/80 backdrop-blur-sm dark:bg-green-950/30 rounded-2xl p-5 border border-green-200/50 dark:border-green-800/50 hover:-translate-y-1 transition-all duration-200">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-green-700 dark:text-green-400">Concluídas</p>
                  <p className="text-2xl font-bold text-green-900 dark:text-green-300">{stats.done}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <Card className="relative bg-white/80 backdrop-blur-sm dark:bg-slate-900/80 border-slate-200/50 dark:border-slate-700/50 shadow-xl">
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-col md:flex-row gap-3 md:items-center justify-between">
                  ...
                  <Input placeholder="Buscar..." value={search} onChange={(e) => setSearch(e.target.value)} className="md:w-64 bg-white/50 dark:bg-slate-800/50" />
                </div>
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-3 text-xs mb-2">
                                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-md font-medium ${task.priority === 'high' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                              ...
                              {task.assignee && (
                                <p className="text-xs text-slate-600 dark:text-slate-400 mb-1">Responsável: {task.assignee}</p>
                              )}
                              {task.description && (
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/30" onClick={() => handleDelete(task.id)}>
              ...
              <div className="relative bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-auto m-4" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="relative p-6 bg-linear-to-r from-blue-600 to-purple-600">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" className="text-white hover:bg-white/20" onClick={() => setIsModalOpen(false)}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
